<?php
	...
	<?php
		...
		$query = "select * from zrealizirani order by id desc limit $page1,5;";
		$result = mysqli_query($db_server,$query);

		if (!$result) die ("Database access failed: " . mysqli_error());
		
		
		$sql = "SELECT * FROM zrealizirani";
	...
	{?>
		...
		<div class="content">
			...
			$('#<?php echo $row['id']?> img').hover(function() {
				$(this).attr('src', 'slike/zgradarstvo/hover2.png');
				}, function() {
	...
	<?php
		$result2 = mysqli_query($db_server,"select * from zrealizirani order by id desc;");
		...
		echo "<div id=brojevi>";
		for($b=1;$b<=$a;$b++){
			?><a onclick="$('article').load('php/zrealizirano.php?page=<?php echo $b; ?>')" href="#vrh" style="color:white;text-decoration:none;background-color:#80df00;margin:2px;padding:2px;"><?php echo $b." ";?></a><?php
		}
		echo "</div>";
	?>
